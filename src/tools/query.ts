...
import { formatQueryResult, formatExplainResult, formatErrorResult } from '../utils/result-formatter';
import { QueryBuilder, ExplainResultParser } from '../utils/query-builder';
import { MCPToolDefinition, QueryDatabaseArgs, ExplainQueryArgs } from '../types/mcp';
...
const queryDatabase = async (args: QueryDatabaseArgs) => {
    ...
    console.error(`🔍 Executing ${dbType?.toUpperCase()} query: ${query.substring(0, 100)}${query.length > 100 ? '...' : ''}`);
    ...
    return formatQueryResult(result, executionTime, dbType!, QUERY_LIMITS.MAX_ROWS);
};
...
const explainQuery = async (args: ExplainQueryArgs) => {
    ...
    console.error(`🔍 Explaining ${dbType?.toUpperCase()} query${analyze ? ' with analysis' : ''}: ${query.substring(0, 100)}${query.length > 100 ? '...' : ''}`);
    ...
    const executionPlan = ExplainResultParser.parseExplainResult(dbType!, result.rows);
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                analyze: {
                    type: 'boolean',
                    description: 'Whether to include actual execution statistics (WARNING: this executes the query). Not supported for SQLite.',
                    default: false,
                },
            },
            required: ['query'],
