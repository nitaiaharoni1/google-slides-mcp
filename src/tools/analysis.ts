...
const getTableStats = async (args: GetTableStatsArgs) => {
  ...
  console.error(
    `📊 Getting table statistics for ${dbType?.toUpperCase()} schema: ${schemaName}`,
  );
  ...
  try {
    ...
    console.error(
      `✅ Retrieved stats for ${tableSizeResult.rows.length} tables`,
    );
    ...
    throw new Error(
      `Failed to get table statistics: ${(error as Error).message}`,
    );
  }
};
...
const analyzeColumn = async (args: AnalyzeColumnArgs) => {
  ...
  console.error(
    `🔍 Analyzing column '${column_name}' in table '${table_name}' for ${dbType?.toUpperCase()}`,
  );
  ...
  try {
    ...
    const columnExists = columnResult.rows.some(
      (row: any) =>
        row.column_name?.toLowerCase() === column_name.toLowerCase(),
    );
    ...
    const statsQuery = QueryBuilder.buildColumnStatsQuery(
      dbType!,
      table_name,
      column_name,
    );
    ...
    const mcvQuery = QueryBuilder.buildMostCommonValuesQuery(
      dbType!,
      table_name,
      column_name,
      10,
...
const getDatabaseInfo = async (args: any) => {
  ...
  console.error(
    `ℹ️  Getting database information for ${dbType?.toUpperCase()}`,
  );
...
export const analysisTools: MCPToolDefinition[] = [
  {
    ...
    description:
      'Get comprehensive statistics about tables including sizes and row counts. Works with PostgreSQL, MySQL, and SQLite.',
    inputSchema: {
      type: 'object',
      properties: {
        schema_name: {
  ...
  },
  {
    name: 'analyze_column',
    description:
      'Analyze a specific column for data quality, distribution, and most common values. Works with PostgreSQL, MySQL, and SQLite.',
    inputSchema: {
      type: 'object',
      properties: {
        table_name: {
  ...
  },
  {
    ...
    description:
      'Get comprehensive database information including version, size, settings, and activity. Works with PostgreSQL, MySQL, and SQLite.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
