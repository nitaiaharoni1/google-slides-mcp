...
const searchTables = async (args: SearchTablesArgs) => {
    ...
    console.error(`🔍 Searching for '${searchTerm}' in ${dbType?.toUpperCase()} database`);
    ...
    const matchingTables = tablesResult.rows.filter((table: any) =>
        table.table_name?.toLowerCase().includes(searchTerm)
    ).slice(0, limit);

    // Search columns in all tables
    const columnMatches = [];
    for (const table of tablesResult.rows.slice(0, 50)) { // Limit to avoid too many queries
        try {
    ...
    console.error(`✅ Found ${matchingTables.length} table matches and ${limitedColumnMatches.length} column matches`);
...
export const discoveryTools: MCPToolDefinition[] = [
    {
        name: 'search_tables',
        description: 'Search for tables and columns by name or pattern. Works with PostgreSQL, MySQL, and SQLite.',
        inputSchema: {
            type: 'object',
            properties: {
                search_term: {
