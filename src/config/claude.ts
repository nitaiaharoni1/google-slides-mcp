...
export function configureClaudeDesktop(databaseUrl?: string): void {
  console.log(
    '🔧 Configuring Claude Desktop for Multi-Database MCP Server...\n',
  );
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        console.log(
          '⚠️  Existing config file has invalid JSON, creating new configuration',
        );
        config = {};
      }
    } else {
    ...
    if (!databaseUrlToUse) {
      ...
      console.log(
        '  PostgreSQL: export DATABASE_URL="postgresql://user:pass@host:5432/db?sslmode=require"',
      );
      console.log(
        '  MySQL:      export DATABASE_URL="mysql://user:pass@host:3306/db"',
      );
    ...
    console.log(
      JSON.stringify(
        {
          mcpServers: { 'multi-database': config.mcpServers['multi-database'] },
        },
        null,
        2,
      ),
    ...
    if (
      ...
      console.log(
        '3. Or edit the config file manually to add your database connection string',
      );
    } else {
      if (databaseUrl) {
      ...
      console.log(
        '3. Start chatting with Claude - it now has access to your database!',
      );
    }
...
export function getClaudeConfigPath(): string {
  ...
  switch (platform) {
    ...
    default:
      throw new Error(
        `Unsupported operating system: ${platform}. Please configure manually.`,
      );
  }
}
...
export function showConfigLocation(): void {
  ...
  try {
    ...
    console.log(
      `🖥️  Operating System: ${platform === 'darwin' ? 'macOS' : platform === 'win32' ? 'Windows' : platform}`,
    );
    ...
    console.log(
      `📄 File exists: ${fileExists ? '✅ Yes' : '❌ No (will be created when you add config)'}`,
    );
    ...
    if (fileExists) {
      try {
        ...
        const hasMultiDatabase =
          config.mcpServers && config.mcpServers['multi-database'];
        console.log(
          `🔗 Multi-Database MCP configured: ${hasMultiDatabase ? '✅ Yes' : '❌ No'}`,
        );
      } catch (error) {
    ...
    if (!process.env.DATABASE_URL) {
      ...
      console.log(
        '   SQLite:     export DATABASE_URL="sqlite://./database.db"',
      );
    }
  } catch (error) {
    console.error(
      '❌ Failed to locate Claude Desktop configuration:',
      (error as Error).message,
    );
    console.error('\n💡 Supported operating systems: macOS, Linux, Windows');
    console.error(
      'For manual configuration, please refer to the Claude Desktop documentation.',
    );
    process.exit(1);
  }
}
