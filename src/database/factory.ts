...
class DatabaseFactory {
  ...
  static detectDatabaseType(connectionString: string): DatabaseType {
    ...
    if (
      lowerStr.startsWith('postgresql://') ||
    ...
    // MySQL detection
    if (lowerStr.startsWith('mysql://') || lowerStr.includes('mysql')) {
      return 'mysql';
    }

    // Snowflake detection
    if (
      lowerStr.startsWith('snowflake://') ||
    ...
    throw new Error(
      `Cannot detect database type from connection string: ${connectionString}`,
    );
  }
  ...
  private static _validateSQLiteString(
    ...
    const cleanPath = connectionString
      .replace(/^sqlite:\/\//, '')
      .replace(/^sqlite:/, '');
    ...
    if (!hasValidExtension && !cleanPath.includes('/')) {
      errors.push(
        `SQLite file should have one of these extensions: ${SQLITE_EXTENSIONS.join(', ')}`,
      );
    }
  }
    ...
    try {
      if (connectionString.startsWith('snowflake://')) {
        ...
        if (!url.hostname || !url.hostname.includes('snowflakecomputing.com')) {
          errors.push(
            'Snowflake connection string must include account.snowflakecomputing.com hostname',
          );
        }
        ...
        if (pathParts.length === 0) {
          errors.push(
            'Snowflake connection string should include database name in path',
          );
        }
