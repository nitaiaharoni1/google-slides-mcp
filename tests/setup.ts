...
function configureSSL() {
  ...
  const isCloudProvider = databaseUrl.includes('ondigitalocean.com') ||
  ...
  if (shouldRelaxSSL && process.env.NODE_TLS_REJECT_UNAUTHORIZED !== '0') {
    ...
    console.warn('⚠️  TLS certificate verification disabled for cloud database in test environment');
    console.warn('   This is safe for testing but should not be used in production');
  }
}
