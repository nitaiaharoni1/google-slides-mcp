...
function runCommand(command: string, description: string): string {
  try {
    ...
    console.error(
      colorize(`✗ ${description} failed: ${(error as Error).message}`, "red")
    );
    throw error;
  }
}
...
function incrementVersion(
  ...
  if (type === "major") {
    return `${String(parts[0] + 1)}.0.0`;
  }
  if (type === "minor") {
    return `${String(parts[0])}.${String(parts[1] + 1)}.0`;
  }
  // type === "patch"
  return `${String(parts[0])}.${String(parts[1])}.${String(parts[2] + 1)}`;
}
...
function validateWorkingDirectory(): void {
  try {
    ...
    console.error(
      colorize(`✗ Git validation failed: ${(error as Error).message}`, "red")
    );
    throw error;
  }
}
...
function main(): void {
  ...
  try {
    ...
    const newVersion =
      options.version || incrementVersion(currentVersion, versionType);
    ...
    if (options.dryRun) {
      console.log(
        colorize("🔍 Dry run mode - no changes will be made", "yellow")
      );
      return;
    }
    ...
    console.log(
      colorize(`✅ Release ${newVersion} completed successfully!`, "green")
    );
    console.log(
      colorize(
        "📝 Don't forget to push with: git push && git push --tags",
        "cyan"
      )
    );
  } catch (error) {
    console.error(
      colorize(`❌ Release failed: ${(error as Error).message}`, "red")
    );
    process.exit(1);
  }
}
...
if (require.main === module) {
  try {
    ...
    console.error(colorize(`❌ Fatal error: ${errorMessage}`, "red"));
    process.exit(1);
  }
}
